...
case class Location(
  ...
  def myArgs: List[Tuple2[String, Object]] =
...
case class CurrentResult(location: Location, current: Current):
  def myArgs: List[Tuple2[String, Object]] = this.current.myArgs ++ this.location.myArgs
...
class JsonParsedCurrent(response: Response) extends JsonParser[CurrentResult]:
  val rawValue: String = response.text()
  ...
  val header: Map[String, Seq[String]] = response.headers
...
case class Astronomy(astro: Astro):
  def myArgs: List[Tuple2[String, Object]] = this.astro.myArgs

case class AstronomyResult(location: Location, astronomy: Astronomy):
  def myArgs: List[Tuple2[String, Object]] = this.location.myArgs ++ this.astronomy.myArgs

class JsonParsedAstronomy(response: Response) extends JsonParser[AstronomyResult]:
  val rawValue: String = response.text()
...
class JsonParsedTimeZone(response: Response) extends JsonParser[Location]:
...
class JsonParsedFootball(response: Response) extends JsonParser[List[Matches]]:
...
class JsonParsedForecast(response: Response) extends JsonParser[List[Hour]]:
